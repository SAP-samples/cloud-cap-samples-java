...
class AdminServiceHandler implements EventHandler {
	...
	public void beforeCreateOrder(Stream<Orders> orders) {
		orders.forEach(order -> {
			...
			order.getItems().forEach(orderItem -> {
				...
				if (bookId == null) {
					// Tip: using static text without localization is still possible in exceptions
					// and messages
				...
				// FIXME this should handle book changes, currently only amount changes are
				// handled
				...
				Result result = db
						.run(Select.from(BOOKS).columns(b -> b.ID(), b -> b.stock(), b -> b.price()).byId(bookId));
	...
	/**
	 * Calculate the total order value preview when deleting an order item from the
	 * order
	 *
	 * @param context
	 */
	...
	private BigDecimal calculateNetAmountInDraft(String orderItemId, Integer newAmount, String newBookId) {
		...
		// get the order item that was updated (to get access to the book price, amount
		// and order total)
		Result result = adminService.run(Select.from(ORDER_ITEMS)
				.columns(o -> o.amount(), o -> o.netAmount(), o -> o.book().expand(b -> b.ID(), b -> b.price()),
	...
	public void addBookToOrder(AddToOrderContext context) {
		...
		List<Orders> orders = adminService
				.run(Select.from(ORDERS).columns(o -> o._all(), o -> o.Items().expand()).where(o -> o.ID().eq(orderId)))
				.listOf(Orders.class);
