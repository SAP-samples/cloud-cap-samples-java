...
public class AdminServiceAddressHandler implements EventHandler {
  ...
  private static final Logger logger = LoggerFactory.getLogger(AdminServiceAddressHandler.class);

  // We are mashing up the AdminService with two other services...
  private final PersistenceService db;
  private final ApiBusinessPartner bupa;

  AdminServiceAddressHandler(
      PersistenceService db, @Qualifier(ApiBusinessPartner_.CDS_NAME) ApiBusinessPartner bupa) {
    this.db = db;
    this.bupa = bupa;
  }
  ...
  public void readAddresses(CdsReadEventContext context) {
    ...
    String businessPartner =
        context.getUserInfo().getAttributeValues("businessPartner").stream()
            .findFirst()
            .orElseThrow(
                () -> new ServiceException(ErrorStatuses.FORBIDDEN, MessageKeys.BUPA_MISSING));
    ...
    ResilienceConfiguration config =
        ResilienceConfiguration.of(AdminServiceAddressHandler.class)
  ...
  public void patchAddressId(EventContext context, Stream<Orders> orders) {
    ...
    orders
        .filter(o -> o.getShippingAddressId() != null)
        .forEach(
            order -> {
  ...
  public void updateBusinessPartnerAddresses(BusinessPartnerChangedContext context) {
    ...
    var replicas =
        db.run(Select.from(ADDRESSES).where(a -> a.businessPartner().eq(businessPartner)));
    if (replicas.rowCount() > 0) {
      ...
      var remoteAddresses =
          bupa.run(Select.from(ADDRESSES).where(a -> a.businessPartner().eq(businessPartner)));
      // update replicas or add tombstone if external address was deleted
      replicas.stream()
          .forEach(
              rep -> {
                Optional<Addresses> matching =
                    remoteAddresses.stream()
