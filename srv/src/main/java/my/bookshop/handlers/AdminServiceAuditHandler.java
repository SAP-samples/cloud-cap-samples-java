...
class AdminServiceAuditHandler implements EventHandler {
  ...
  private void auditCfgChange(
      final Action action, final ConfigChange cfgChange, EventContext context) {
  ...
  private static ConfigChange createConfigChange(Orders orders, Orders oldOrders) {
    ChangedAttribute currencyCodeAttr =
        createChangedAttribute(
            orders != null ? orders.getCurrencyCode() : null,
